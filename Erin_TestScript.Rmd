...
### DEFINTE THE FOLLOWING VARIABLES FOR YOUR ANALYSIS
...
species_list <- as.data.frame(species_list[sample(1:dim(species_list)[1], 20),]) 
colnames(species_list)<- c("search_name") # subset for testing
order_list <- read.csv("workingfiles/MetazoaSpeciesByOrder_2023-09-19.csv") # list of vertebrate orders
order_list <- order_list[sample(1:dim(order_list)[1],10),] # subset for testing
```
...
order_seqs <- as.data.frame(matrix(nrow=1, ncol=6, dimnames=list(NULL, c("phylum", "class", "order", "species_id", "ids_mitogenome","ids_target")))) # create missing orders representatives skeleton
...
order_seqs <- na.omit(order_seqs) #substract the first row because its all NAs
...
while (m <= dim(a01_NAMES)[1]) { #for every good species name
    ...
    } else if (a01_NAMES$n_mitogenome[m]>20) { #if >20 mitogenomes, sample 20 randomly
    ...
    if (mito_ids[1] != "na") { #if there are mitogenome ids for species m
        ...
        if (class(mito_accessions) != "try-catch"){ #if no api error
            ...
            for (n in 1:length(mito_accessions)){ # loop through and scrape each mitogenome accession
                ...
                target_feature <- which(otherFeatures(gb)$product %in% as.character(target_locus_synonyms$Name)) # find target locus annotation metadata 
                new_row <- c(paste("Unparsed mitochondrion", mito_accessions[n], sep=" "), "na", mito_accessions[n], "scrape", species=a01_NAMES$search_name[m])
                if(length(target_feature) > 0) { # if target feature is found
                    target_range <- otherFeatures(gb)@ranges[target_feature] #extract the target range info
                    target_strand <-  otherFeatures(gb)@strand[target_feature] #extract the target strand info (+ or -)
                    ...
                    new_row <- c(paste(names(target_seq),"mitochondrion", mito_accessions[n], sep=" "), paste(target_seq), mito_accessions[n], "scrape", species=a01_NAMES$search_name[m]) #update information
                    }
                }
                else if (is_mtgene==TRUE){ # if target locus is a gene, look in gene()
                 target_feature <- which(genes(gb)$gene %in% as.character(target_locus_synonyms$Name)) # find target locus annotation metadata 
                 new_row <- c(paste("Unparsed mitochondrion", mito_accessions[n], sep=" "), "na", mito_accessions[n], "scrape", species=a01_NAMES$search_name[m])
                if(length(target_feature) > 0) { # if target feature is found
                    target_range <- otherFeatures(gb)@ranges[target_feature] #extract the target range info
                    target_strand <-  otherFeatures(gb)@strand[target_feature] #extract the target strand info (+ or -)
...
## De-duplicate the reference database, but keep track of duplicate accessions/scrapes
```{r remove_duplicates}
a03_UNIQUEDB <- a02_REFDB[!duplicated(a02_REFDB$sequence),] #remove duplicates
for (i in  1:dim(a03_UNIQUEDB)[1]){ # for every row in the unique db file
  dups <- subset(a02_REFDB, sequence == a03_UNIQUEDB$sequence[i]) # find identical sequences in the fill db file 
  a03_UNIQUEDB$duplicate_accessions[i] <- paste(dups$seq_accession, collapse = "|") # paste all those accessions together into a new unique db field
  a03_UNIQUEDB$duplicate_species[i] <- paste(dups[!duplicated(dups$species),"species"], collapse = "|")
}
